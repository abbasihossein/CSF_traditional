...
if exist(fullfile(ExperimentCodes_path, 'SavedData'), 'dir') ~= 7
    % If the folder does not exist, create it
    SaveDirectory = mkdir(ExperimentCodes_path, 'SavedData');
    disp(['Folder "', 'SavedData', '" created in directory "', ExperimentCodes_path, '"']);
else
    SaveDirectory = [ExperimentCodes_path, '\SavedData'];
    disp(['Folder "', 'SavedData', '" already exists in directory "', ExperimentCodes_path, '"']);
end

cd(fullfile(ExperimentCodes_path, 'SavedData'));
...
if ~test
    ...
    while ~Pass
        ...
        definput = {'Hossein Abbasi','e.g. 11','60','80','3','5'};
...
if do_the_practice
    ...
    while ~done
        [done,New_vbl] = Practice(screen_info,stimuli_info,Practice_info);
    end
...
for BLOCK = 1:length(stimuli_info.List_of_Freqs_cpd)
    ...
    for trl = 1:nTrials
        ...
        if trl == 1
            This_Contrast = stimuli_info.Start_contrast;
        end
...
%                 
        ...
        TRGs = [screen_info.screenXpixels/5,screen_info.screenYpixels/4;...
            ...
            screen_info.screenXpixels*(3/5),screen_info.screenYpixels/4;...
            screen_info.screenXpixels*(4/5),screen_info.screenYpixels/4;...
            screen_info.screenXpixels/5,screen_info.screenYpixels*(3/4);...
            screen_info.screenXpixels*(2/5),screen_info.screenYpixels*(3/4);...
            screen_info.screenXpixels*(3/5),screen_info.screenYpixels*(3/4);...
            screen_info.screenXpixels*(4/5),screen_info.screenYpixels*(3/4)];
        for stm = 1:8
            Target_Locations1(stm,:) = [TRGs(stm,1)-125,TRGs(stm,2)-125,TRGs(stm,1)+125,TRGs(stm,2)+125];
        end
        
        
        for frm = 1:stimuli_info.stimDuration_Frames
            for stm = 1:8
                stimuli_info.propertiesMat = [stimuli_info.phase, FRQ(stm), stimuli_info.sigma, CSs(stm), stimuli_info.aspectRatio, 0, 0, 0];
                
                Screen('DrawTextures', screen_info.window, stimuli_info.gabortex, [], Target_Locations1(stm,:), stimuli_info.orientation, [], [], [], [],...
                    kPsychDontDoRotation, stimuli_info.propertiesMat');
                
                Screen('FrameRect', screen_info.window, stimuli_info.frameColor,Target_Locations1(stm,:), stimuli_info.borderThickness);
            end
            screen_info.vbl  = Screen('Flip', screen_info.window, screen_info.vbl + (screen_info.waitframes - 0.5) * screen_info.ifi);
...
%         end
    ...
    if REVexist == 1 && length(Behavioral_response.Revs{BLOCK})>2 
        Switches.USED{BLOCK} = Behavioral_response.Revs{BLOCK}(2:end-1);
